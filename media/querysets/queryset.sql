...
case
	when DATEDIFF(recisao, admissao) > 365 then
		case
			when DATEDIFF(recisao, cast(concat(year(curdate()), '-01-01') as date)) <   14 then 0
			when DATEDIFF(recisao, cast(concat(year(curdate()), '-01-01') as date)) >=  15 and DATEDIFF(recisao, cast(concat(year(curdate()), '-01-01') as date)) <  45 then (main.premio / 12) * 1
            when DATEDIFF(recisao, cast(concat(year(curdate()), '-01-01') as date)) >=  45 and DATEDIFF(recisao, cast(concat(year(curdate()), '-01-01') as date)) <  75 then (main.premio / 12) * 2
            when DATEDIFF(recisao, cast(concat(year(curdate()), '-01-01') as date)) >=  75 and DATEDIFF(recisao, cast(concat(year(curdate()), '-01-01') as date)) < 105 then (main.premio / 12) * 3
            when DATEDIFF(recisao, cast(concat(year(curdate()), '-01-01') as date)) >= 105 and DATEDIFF(recisao, cast(concat(year(curdate()), '-01-01') as date)) < 135 then (main.premio / 12) * 4
            when DATEDIFF(recisao, cast(concat(year(curdate()), '-01-01') as date)) >= 135 and DATEDIFF(recisao, cast(concat(year(curdate()), '-01-01') as date)) < 165 then (main.premio / 12) * 5
            when DATEDIFF(recisao, cast(concat(year(curdate()), '-01-01') as date)) >= 165 and DATEDIFF(recisao, cast(concat(year(curdate()), '-01-01') as date)) < 195 then (main.premio / 12) * 6
            when DATEDIFF(recisao, cast(concat(year(curdate()), '-01-01') as date)) >= 195 and DATEDIFF(recisao, cast(concat(year(curdate()), '-01-01') as date)) < 225 then (main.premio / 12) * 7
            when DATEDIFF(recisao, cast(concat(year(curdate()), '-01-01') as date)) >= 225 and DATEDIFF(recisao, cast(concat(year(curdate()), '-01-01') as date)) < 255 then (main.premio / 12) * 8
            when DATEDIFF(recisao, cast(concat(year(curdate()), '-01-01') as date)) >= 255 and DATEDIFF(recisao, cast(concat(year(curdate()), '-01-01') as date)) < 285 then (main.premio / 12) * 9
            when DATEDIFF(recisao, cast(concat(year(curdate()), '-01-01') as date)) >= 285 and DATEDIFF(recisao, cast(concat(year(curdate()), '-01-01') as date)) < 315 then (main.premio / 12) * 10
            when DATEDIFF(recisao, cast(concat(year(curdate()), '-01-01') as date)) >= 315 and DATEDIFF(recisao, cast(concat(year(curdate()), '-01-01') as date)) < 345 then (main.premio / 12) * 11
            else main.premio 
		end
	else
        case
			when DATEDIFF(recisao, admissao) <   14 then 0
			when DATEDIFF(recisao, admissao) >=  15 and DATEDIFF(recisao, admissao) <  45 then (main.premio / 12) * 1
            when DATEDIFF(recisao, admissao) >=  45 and DATEDIFF(recisao, admissao) <  75 then (main.premio / 12) * 2
            when DATEDIFF(recisao, admissao) >=  75 and DATEDIFF(recisao, admissao) < 105 then (main.premio / 12) * 3
            when DATEDIFF(recisao, admissao) >= 105 and DATEDIFF(recisao, admissao) < 135 then (main.premio / 12) * 4
            when DATEDIFF(recisao, admissao) >= 135 and DATEDIFF(recisao, admissao) < 165 then (main.premio / 12) * 5
            when DATEDIFF(recisao, admissao) >= 165 and DATEDIFF(recisao, admissao) < 195 then (main.premio / 12) * 6
            when DATEDIFF(recisao, admissao) >= 195 and DATEDIFF(recisao, admissao) < 225 then (main.premio / 12) * 7
            when DATEDIFF(recisao, admissao) >= 225 and DATEDIFF(recisao, admissao) < 255 then (main.premio / 12) * 8
            when DATEDIFF(recisao, admissao) >= 255 and DATEDIFF(recisao, admissao) < 285 then (main.premio / 12) * 9
            when DATEDIFF(recisao, admissao) >= 285 and DATEDIFF(recisao, admissao) < 315 then (main.premio / 12) * 10
            when DATEDIFF(recisao, admissao) >= 315 and DATEDIFF(recisao, admissao) < 345 then (main.premio / 12) * 11
            else main.premio 
		end
end as premio_total
...
from (
    ...
    select
    xx.matricula,
    cast(xx.admissao as date) as admissao,
    cast(xx.recisao as date) as recisao,
    xx.funcionario,
    xx.cargo,
    xx.setor,
    xx.tipo,
    ...
    xx.salario * (select csf.valor from main_classificacao csf where xx.total_margem between csf.minimo and csf.maximo and csf.regua_id = xx.regua_id)  as premio
    ...
    where year(xx.recisao) >= year(curdate()) or xx.recisao is null or xx.recisao = ''

) main order by main.funcionario;